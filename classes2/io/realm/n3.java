// 
// Decompiled by Procyon v0.5.36
// 

package io.realm;

import io.realm.exceptions.RealmException;
import java.util.Iterator;
import io.realm.internal.OsList;
import io.realm.internal.OsObject;
import io.realm.internal.Table;
import android.annotation.TargetApi;
import java.io.IOException;
import android.util.JsonToken;
import android.util.JsonReader;
import org.json.JSONException;
import org.json.JSONArray;
import java.util.List;
import java.util.ArrayList;
import org.json.JSONObject;
import io.realm.internal.OsSchemaInfo;
import io.realm.internal.c;
import java.util.Collections;
import io.realm.internal.r;
import io.realm.internal.objectstore.OsObjectBuilder;
import java.util.Set;
import java.util.Map;
import u4.b;
import io.realm.internal.OsObjectSchemaInfo;
import io.realm.internal.p;
import u4.a;

public class n3 extends u4.a implements p, o3
{
    private static final String R = "";
    private static final OsObjectSchemaInfo S;
    private b O;
    private c0<u4.a> P;
    private l0<u4.b> Q;
    
    static {
        S = G8();
    }
    
    n3() {
        this.P.p();
    }
    
    public static u4.a C8(final f0 f0, final b b, final u4.a a, final boolean b2, final Map<n0, p> map, final Set<q> set) {
        final n3 n3 = map.get(a);
        if (n3 != null) {
            return n3;
        }
        final OsObjectBuilder osObjectBuilder = new OsObjectBuilder(f0.x2(u4.a.class), set);
        osObjectBuilder.L(b.e, a.a());
        osObjectBuilder.d0(b.f, a.g());
        osObjectBuilder.L(b.g, a.d());
        osObjectBuilder.L(b.h, a.c());
        final n3 p6 = P8(f0, osObjectBuilder.f0());
        map.put(a, p6);
        final l0<u4.b> h7 = a.H7();
        if (h7 != null) {
            final l0<u4.b> h8 = p6.H7();
            h8.clear();
            for (int i = 0; i < h7.size(); ++i) {
                final u4.b b3 = h7.get(i);
                u4.b h9 = (u4.b)map.get(b3);
                if (h9 == null) {
                    h9 = l3.H8(f0, (l3.b)f0.H().j(u4.b.class), b3, b2, map, set);
                }
                h8.add(h9);
            }
        }
        return p6;
    }
    
    public static u4.a D8(final f0 f0, final b b, final u4.a a, final boolean b2, final Map<n0, p> map, final Set<q> set) {
        if (a instanceof p && !p0.h8(a)) {
            final p p6 = (p)a;
            if (p6.S6().f() != null) {
                final io.realm.a f2 = p6.S6().f();
                if (f2.H != f0.H) {
                    throw new IllegalArgumentException("Objects which belong to Realm instances in other threads cannot be copied into this Realm instance.");
                }
                if (f2.getPath().equals(f0.getPath())) {
                    return a;
                }
            }
        }
        final io.realm.a.h h = a.W.get();
        final p p7 = map.get(a);
        if (p7 != null) {
            return (u4.a)p7;
        }
        final p p8 = null;
        Object x2 = null;
        boolean b3 = false;
        Label_0232: {
            if (b2) {
                x2 = f0.x2(u4.a.class);
                final long r = ((Table)x2).r(b.e, a.a());
                if (r == -1L) {
                    b3 = false;
                    x2 = p8;
                    break Label_0232;
                }
                try {
                    h.g(f0, ((Table)x2).R(r), b, false, Collections.emptyList());
                    x2 = new n3();
                    map.put(a, (p)x2);
                }
                finally {
                    h.a();
                }
            }
            b3 = b2;
        }
        u4.a a2;
        if (b3) {
            a2 = Q8(f0, b, (u4.a)x2, a, map, set);
        }
        else {
            a2 = C8(f0, b, a, b2, map, set);
        }
        return a2;
    }
    
    public static b E8(final OsSchemaInfo osSchemaInfo) {
        return new b(osSchemaInfo);
    }
    
    public static u4.a F8(final u4.a a, final int a2, final int n, final Map<n0, p.a<n0>> map) {
        if (a2 <= n && a != null) {
            final p.a a3 = map.get(a);
            u4.a a4;
            if (a3 == null) {
                a4 = new u4.a();
                map.put(a, (p.a<n0>)new p.a(a2, a4));
            }
            else {
                if (a2 >= a3.a) {
                    return (u4.a)a3.b;
                }
                a4 = (u4.a)a3.b;
                a3.a = a2;
            }
            a4.b(a.a());
            a4.e(a.g());
            a4.h(a.d());
            a4.i(a.c());
            if (a2 == n) {
                a4.J5(null);
            }
            else {
                final l0<u4.b> h7 = a.H7();
                final l0<u4.b> l0 = new l0<u4.b>();
                a4.J5(l0);
                for (int size = h7.size(), i = 0; i < size; ++i) {
                    l0.add(l3.J8(h7.get(i), a2 + 1, n, map));
                }
            }
            return a4;
        }
        return null;
    }
    
    private static OsObjectSchemaInfo G8() {
        final OsObjectSchemaInfo.b b = new OsObjectSchemaInfo.b("", "RealmParentDay", false, 5, 0);
        final RealmFieldType integer = RealmFieldType.INTEGER;
        b.c("", "id", integer, true, false, true);
        b.c("", "name", RealmFieldType.STRING, false, false, true);
        b.c("", "start", integer, false, false, true);
        b.c("", "end", integer, false, false, true);
        b.b("", "appointments", RealmFieldType.LIST, "RealmParentDayAppointment");
        return b.e();
    }
    
    public static u4.a H8(final f0 f0, final JSONObject jsonObject, final boolean b) throws JSONException {
        final ArrayList<String> list = new ArrayList<String>(1);
        Object x2 = null;
        Label_0146: {
            if (b) {
                x2 = f0.x2(u4.a.class);
                final long e = ((b)f0.H().j(u4.a.class)).e;
                long r;
                if (!jsonObject.isNull("id")) {
                    r = ((Table)x2).r(e, jsonObject.getLong("id"));
                }
                else {
                    r = -1L;
                }
                if (r != -1L) {
                    final io.realm.a.h h = io.realm.a.W.get();
                    try {
                        h.g(f0, ((Table)x2).R(r), f0.H().j(u4.a.class), false, Collections.emptyList());
                        x2 = new n3();
                        break Label_0146;
                    }
                    finally {
                        h.a();
                    }
                }
            }
            x2 = null;
        }
        Object o = x2;
        if (x2 == null) {
            if (jsonObject.has("appointments")) {
                list.add("appointments");
            }
            if (!jsonObject.has("id")) {
                throw new IllegalArgumentException("JSON object doesn't have the primary key field 'id'.");
            }
            n3 n3;
            if (jsonObject.isNull("id")) {
                n3 = f0.p1((Class<n3>)u4.a.class, null, true, list);
            }
            else {
                n3 = f0.p1((Class<n3>)u4.a.class, jsonObject.getLong("id"), true, list);
            }
            o = n3;
        }
        if (jsonObject.has("name")) {
            if (jsonObject.isNull("name")) {
                ((o3)o).e(null);
            }
            else {
                ((o3)o).e(jsonObject.getString("name"));
            }
        }
        if (jsonObject.has("start")) {
            if (jsonObject.isNull("start")) {
                throw new IllegalArgumentException("Trying to set non-nullable field 'start' to null.");
            }
            ((o3)o).h(jsonObject.getLong("start"));
        }
        if (jsonObject.has("end")) {
            if (jsonObject.isNull("end")) {
                throw new IllegalArgumentException("Trying to set non-nullable field 'end' to null.");
            }
            ((o3)o).i(jsonObject.getLong("end"));
        }
        if (jsonObject.has("appointments")) {
            if (jsonObject.isNull("appointments")) {
                ((o3)o).J5(null);
            }
            else {
                ((o3)o).H7().clear();
                final JSONArray jsonArray = jsonObject.getJSONArray("appointments");
                for (int i = 0; i < jsonArray.length(); ++i) {
                    ((o3)o).H7().add(l3.L8(f0, jsonArray.getJSONObject(i), b));
                }
            }
        }
        return (u4.a)o;
    }
    
    @TargetApi(11)
    public static u4.a I8(final f0 f0, final JsonReader jsonReader) throws IOException {
        final u4.a a = new u4.a();
        jsonReader.beginObject();
        boolean b = false;
        while (jsonReader.hasNext()) {
            final String nextName = jsonReader.nextName();
            if (nextName.equals("id")) {
                if (jsonReader.peek() == JsonToken.NULL) {
                    jsonReader.skipValue();
                    throw new IllegalArgumentException("Trying to set non-nullable field 'id' to null.");
                }
                a.b(jsonReader.nextLong());
                b = true;
            }
            else if (nextName.equals("name")) {
                if (jsonReader.peek() != JsonToken.NULL) {
                    a.e(jsonReader.nextString());
                }
                else {
                    jsonReader.skipValue();
                    a.e(null);
                }
            }
            else if (nextName.equals("start")) {
                if (jsonReader.peek() == JsonToken.NULL) {
                    jsonReader.skipValue();
                    throw new IllegalArgumentException("Trying to set non-nullable field 'start' to null.");
                }
                a.h(jsonReader.nextLong());
            }
            else if (nextName.equals("end")) {
                if (jsonReader.peek() == JsonToken.NULL) {
                    jsonReader.skipValue();
                    throw new IllegalArgumentException("Trying to set non-nullable field 'end' to null.");
                }
                a.i(jsonReader.nextLong());
            }
            else if (nextName.equals("appointments")) {
                if (jsonReader.peek() == JsonToken.NULL) {
                    jsonReader.skipValue();
                    a.J5(null);
                }
                else {
                    a.J5(new l0<u4.b>());
                    jsonReader.beginArray();
                    while (jsonReader.hasNext()) {
                        a.H7().add(l3.M8(f0, jsonReader));
                    }
                    jsonReader.endArray();
                }
            }
            else {
                jsonReader.skipValue();
            }
        }
        jsonReader.endObject();
        if (b) {
            return f0.G0(a, new q[0]);
        }
        throw new IllegalArgumentException("JSON object doesn't have the primary key field 'id'.");
    }
    
    public static OsObjectSchemaInfo J8() {
        return n3.S;
    }
    
    public static String K8() {
        return "RealmParentDay";
    }
    
    public static long L8(final f0 f0, final u4.a a, final Map<n0, Long> map) {
        if (a instanceof p && !p0.h8(a)) {
            final p p3 = (p)a;
            if (p3.S6().f() != null && p3.S6().f().getPath().equals(f0.getPath())) {
                return p3.S6().g().l0();
            }
        }
        final Table x2 = f0.x2(u4.a.class);
        final long nativePtr = x2.getNativePtr();
        final b b = (b)f0.H().j(u4.a.class);
        final long e = b.e;
        final Long value = a.a();
        long l;
        if (value != null) {
            l = Table.nativeFindFirstInt(nativePtr, e, a.a());
        }
        else {
            l = -1L;
        }
        if (l == -1L) {
            l = OsObject.createRowWithPrimaryKey(x2, e, a.a());
        }
        else {
            Table.v0(value);
        }
        map.put(a, l);
        final String g = a.g();
        if (g != null) {
            Table.nativeSetString(nativePtr, b.f, l, g, false);
        }
        Table.nativeSetLong(nativePtr, b.g, l, a.d(), false);
        Table.nativeSetLong(nativePtr, b.h, l, a.c(), false);
        final l0<u4.b> h7 = a.H7();
        if (h7 != null) {
            final OsList list = new OsList(x2.R(l), b.i);
            for (final u4.b b2 : h7) {
                Long value2;
                if ((value2 = map.get(b2)) == null) {
                    value2 = l3.P8(f0, b2, map);
                }
                list.l(value2);
            }
        }
        return l;
    }
    
    public static void M8(final f0 f0, final Iterator<? extends n0> iterator, final Map<n0, Long> map) {
        final Table x2 = f0.x2(u4.a.class);
        final long nativePtr = x2.getNativePtr();
        final b b = (b)f0.H().j(u4.a.class);
        final long e = b.e;
        while (iterator.hasNext()) {
            final u4.a a = (u4.a)iterator.next();
            if (map.containsKey(a)) {
                continue;
            }
            if (a instanceof p && !p0.h8(a)) {
                final p p3 = (p)a;
                if (p3.S6().f() != null && p3.S6().f().getPath().equals(f0.getPath())) {
                    map.put(a, p3.S6().g().l0());
                    continue;
                }
            }
            final Long value = a.a();
            long l;
            if (value != null) {
                l = Table.nativeFindFirstInt(nativePtr, e, a.a());
            }
            else {
                l = -1L;
            }
            if (l == -1L) {
                l = OsObject.createRowWithPrimaryKey(x2, e, a.a());
            }
            else {
                Table.v0(value);
            }
            map.put(a, l);
            final String g = a.g();
            if (g != null) {
                Table.nativeSetString(nativePtr, b.f, l, g, false);
            }
            Table.nativeSetLong(nativePtr, b.g, l, a.d(), false);
            Table.nativeSetLong(nativePtr, b.h, l, a.c(), false);
            final l0<u4.b> h7 = a.H7();
            if (h7 == null) {
                continue;
            }
            final OsList list = new OsList(x2.R(l), b.i);
            for (final u4.b b2 : h7) {
                Long value2;
                if ((value2 = map.get(b2)) == null) {
                    value2 = l3.P8(f0, b2, map);
                }
                list.l(value2);
            }
        }
    }
    
    public static long N8(final f0 f0, final u4.a a, final Map<n0, Long> map) {
        if (a instanceof p && !p0.h8(a)) {
            final p p3 = (p)a;
            if (p3.S6().f() != null && p3.S6().f().getPath().equals(f0.getPath())) {
                return p3.S6().g().l0();
            }
        }
        final Table x2 = f0.x2(u4.a.class);
        final long nativePtr = x2.getNativePtr();
        final b b = (b)f0.H().j(u4.a.class);
        final long e = b.e;
        long nativeFindFirstInt;
        if (Long.valueOf(a.a()) != null) {
            nativeFindFirstInt = Table.nativeFindFirstInt(nativePtr, e, a.a());
        }
        else {
            nativeFindFirstInt = -1L;
        }
        long rowWithPrimaryKey = nativeFindFirstInt;
        if (nativeFindFirstInt == -1L) {
            rowWithPrimaryKey = OsObject.createRowWithPrimaryKey(x2, e, a.a());
        }
        map.put(a, rowWithPrimaryKey);
        final String g = a.g();
        if (g != null) {
            Table.nativeSetString(nativePtr, b.f, rowWithPrimaryKey, g, false);
        }
        else {
            Table.nativeSetNull(nativePtr, b.f, rowWithPrimaryKey, false);
        }
        Table.nativeSetLong(nativePtr, b.g, rowWithPrimaryKey, a.d(), false);
        Table.nativeSetLong(nativePtr, b.h, rowWithPrimaryKey, a.c(), false);
        final OsList list = new OsList(x2.R(rowWithPrimaryKey), b.i);
        final l0<u4.b> h7 = a.H7();
        if (h7 != null && h7.size() == list.b0()) {
            for (int size = h7.size(), i = 0; i < size; ++i) {
                final u4.b b2 = h7.get(i);
                Long value;
                if ((value = map.get(b2)) == null) {
                    value = l3.R8(f0, b2, map);
                }
                list.Z(i, value);
            }
        }
        else {
            list.M();
            if (h7 != null) {
                for (final u4.b b3 : h7) {
                    Long value2;
                    if ((value2 = map.get(b3)) == null) {
                        value2 = l3.R8(f0, b3, map);
                    }
                    list.l(value2);
                }
            }
        }
        return rowWithPrimaryKey;
    }
    
    public static void O8(final f0 f0, final Iterator<? extends n0> iterator, final Map<n0, Long> map) {
        final Table x2 = f0.x2(u4.a.class);
        final long nativePtr = x2.getNativePtr();
        final b b = (b)f0.H().j(u4.a.class);
        final long e = b.e;
        while (iterator.hasNext()) {
            final u4.a a = (u4.a)iterator.next();
            if (map.containsKey(a)) {
                continue;
            }
            if (a instanceof p && !p0.h8(a)) {
                final p p3 = (p)a;
                if (p3.S6().f() != null && p3.S6().f().getPath().equals(f0.getPath())) {
                    map.put(a, p3.S6().g().l0());
                    continue;
                }
            }
            long nativeFindFirstInt;
            if (Long.valueOf(a.a()) != null) {
                nativeFindFirstInt = Table.nativeFindFirstInt(nativePtr, e, a.a());
            }
            else {
                nativeFindFirstInt = -1L;
            }
            long rowWithPrimaryKey = nativeFindFirstInt;
            if (nativeFindFirstInt == -1L) {
                rowWithPrimaryKey = OsObject.createRowWithPrimaryKey(x2, e, a.a());
            }
            map.put(a, rowWithPrimaryKey);
            final String g = a.g();
            if (g != null) {
                Table.nativeSetString(nativePtr, b.f, rowWithPrimaryKey, g, false);
            }
            else {
                Table.nativeSetNull(nativePtr, b.f, rowWithPrimaryKey, false);
            }
            Table.nativeSetLong(nativePtr, b.g, rowWithPrimaryKey, a.d(), false);
            Table.nativeSetLong(nativePtr, b.h, rowWithPrimaryKey, a.c(), false);
            final OsList list = new OsList(x2.R(rowWithPrimaryKey), b.i);
            final l0<u4.b> h7 = a.H7();
            if (h7 != null && h7.size() == list.b0()) {
                for (int size = h7.size(), i = 0; i < size; ++i) {
                    final u4.b b2 = h7.get(i);
                    Long value;
                    if ((value = map.get(b2)) == null) {
                        value = l3.R8(f0, b2, map);
                    }
                    list.Z(i, value);
                }
            }
            else {
                list.M();
                if (h7 == null) {
                    continue;
                }
                for (final u4.b b3 : h7) {
                    Long value2;
                    if ((value2 = map.get(b3)) == null) {
                        value2 = l3.R8(f0, b3, map);
                    }
                    list.l(value2);
                }
            }
        }
    }
    
    static n3 P8(final io.realm.a a, final r r) {
        final io.realm.a.h h = a.W.get();
        h.g(a, r, a.H().j(u4.a.class), false, Collections.emptyList());
        final n3 n3 = new n3();
        h.a();
        return n3;
    }
    
    static u4.a Q8(final f0 f0, final b b, final u4.a a, final u4.a a2, final Map<n0, p> map, final Set<q> set) {
        final OsObjectBuilder osObjectBuilder = new OsObjectBuilder(f0.x2(u4.a.class), set);
        osObjectBuilder.L(b.e, a2.a());
        osObjectBuilder.d0(b.f, a2.g());
        osObjectBuilder.L(b.g, a2.d());
        osObjectBuilder.L(b.h, a2.c());
        final l0<u4.b> h7 = a2.H7();
        if (h7 != null) {
            final l0<n0> l0 = new l0<n0>();
            for (int i = 0; i < h7.size(); ++i) {
                final u4.b b2 = h7.get(i);
                u4.b h8 = (u4.b)map.get(b2);
                if (h8 == null) {
                    h8 = l3.H8(f0, (l3.b)f0.H().j(u4.b.class), b2, true, map, set);
                }
                l0.add(h8);
            }
            osObjectBuilder.Z(b.i, l0);
        }
        else {
            osObjectBuilder.Z(b.i, new l0<n0>());
        }
        osObjectBuilder.i0();
        return a;
    }
    
    @Override
    public l0<u4.b> H7() {
        this.P.f().h();
        final l0<u4.b> q = this.Q;
        if (q != null) {
            return q;
        }
        return this.Q = new l0<u4.b>(u4.b.class, this.P.g().p(this.O.i), this.P.f());
    }
    
    @Override
    public void J5(final l0<u4.b> l0) {
        final boolean i = this.P.i();
        final int n = 0;
        int j = 0;
        l0<u4.b> l2 = l0;
        if (i) {
            if (!this.P.d()) {
                return;
            }
            if (this.P.e().contains("appointments")) {
                return;
            }
            if ((l2 = l0) != null) {
                l2 = l0;
                if (!l0.G0()) {
                    final f0 f0 = (f0)this.P.f();
                    l2 = new l0<u4.b>();
                    for (final u4.b b : l0) {
                        u4.b b2;
                        if ((b2 = b) != null) {
                            if (p0.j8(b)) {
                                b2 = b;
                            }
                            else {
                                b2 = f0.G0(b, new q[0]);
                            }
                        }
                        l2.add(b2);
                    }
                }
            }
        }
        this.P.f().h();
        final OsList p = this.P.g().p(this.O.i);
        if (l2 != null && l2.size() == p.b0()) {
            while (j < l2.size()) {
                final u4.b b3 = l2.get(j);
                this.P.c(b3);
                p.Z(j, ((p)b3).S6().g().l0());
                ++j;
            }
        }
        else {
            p.M();
            if (l2 == null) {
                return;
            }
            for (int size = l2.size(), k = n; k < size; ++k) {
                final u4.b b4 = l2.get(k);
                this.P.c(b4);
                p.l(((p)b4).S6().g().l0());
            }
        }
    }
    
    @Override
    public c0<?> S6() {
        return this.P;
    }
    
    @Override
    public long a() {
        this.P.f().h();
        return this.P.g().m(this.O.e);
    }
    
    @Override
    public void b(final long n) {
        if (this.P.i()) {
            return;
        }
        this.P.f().h();
        throw new RealmException("Primary key field 'id' cannot be changed after object was created.");
    }
    
    @Override
    public long c() {
        this.P.f().h();
        return this.P.g().m(this.O.h);
    }
    
    @Override
    public long d() {
        this.P.f().h();
        return this.P.g().m(this.O.g);
    }
    
    @Override
    public void e(final String s) {
        if (this.P.i()) {
            if (!this.P.d()) {
                return;
            }
            final r g = this.P.g();
            if (s != null) {
                g.g().t0(this.O.f, g.l0(), s, true);
                return;
            }
            throw new IllegalArgumentException("Trying to set non-nullable field 'name' to null.");
        }
        else {
            this.P.f().h();
            if (s != null) {
                this.P.g().d(this.O.f, s);
                return;
            }
            throw new IllegalArgumentException("Trying to set non-nullable field 'name' to null.");
        }
    }
    
    @Override
    public boolean equals(final Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || this.getClass() != o.getClass()) {
            return false;
        }
        final n3 n3 = (n3)o;
        final io.realm.a f = this.P.f();
        final io.realm.a f2 = n3.P.f();
        final String path = f.getPath();
        final String path2 = f2.getPath();
        Label_0083: {
            if (path != null) {
                if (path.equals(path2)) {
                    break Label_0083;
                }
            }
            else if (path2 == null) {
                break Label_0083;
            }
            return false;
        }
        if (f.N() != f2.N()) {
            return false;
        }
        if (!f.K.getVersionID().equals(f2.K.getVersionID())) {
            return false;
        }
        final String m = this.P.g().g().M();
        final String i = n3.P.g().g().M();
        if (m != null) {
            if (m.equals(i)) {
                return this.P.g().l0() == n3.P.g().l0();
            }
        }
        else if (i == null) {
            return this.P.g().l0() == n3.P.g().l0();
        }
        return false;
    }
    
    @Override
    public String g() {
        this.P.f().h();
        return this.P.g().U(this.O.f);
    }
    
    @Override
    public void h(final long n) {
        if (!this.P.i()) {
            this.P.f().h();
            this.P.g().s(this.O.g, n);
            return;
        }
        if (!this.P.d()) {
            return;
        }
        final r g = this.P.g();
        g.g().q0(this.O.g, g.l0(), n, true);
    }
    
    @Override
    public int hashCode() {
        final String path = this.P.f().getPath();
        final String m = this.P.g().g().M();
        final long l0 = this.P.g().l0();
        int hashCode = 0;
        int hashCode2;
        if (path != null) {
            hashCode2 = path.hashCode();
        }
        else {
            hashCode2 = 0;
        }
        if (m != null) {
            hashCode = m.hashCode();
        }
        return ((527 + hashCode2) * 31 + hashCode) * 31 + (int)(l0 >>> 32 ^ l0);
    }
    
    @Override
    public void i(final long n) {
        if (!this.P.i()) {
            this.P.f().h();
            this.P.g().s(this.O.h, n);
            return;
        }
        if (!this.P.d()) {
            return;
        }
        final r g = this.P.g();
        g.g().q0(this.O.h, g.l0(), n, true);
    }
    
    @Override
    public String toString() {
        if (!p0.k8(this)) {
            return "Invalid object";
        }
        final StringBuilder sb = new StringBuilder("RealmParentDay = proxy[");
        sb.append("{id:");
        sb.append(this.a());
        sb.append("}");
        sb.append(",");
        sb.append("{name:");
        sb.append(this.g());
        sb.append("}");
        sb.append(",");
        sb.append("{start:");
        sb.append(this.d());
        sb.append("}");
        sb.append(",");
        sb.append("{end:");
        sb.append(this.c());
        sb.append("}");
        sb.append(",");
        sb.append("{appointments:");
        sb.append("RealmList<RealmParentDayAppointment>[");
        sb.append(this.H7().size());
        sb.append("]");
        sb.append("}");
        sb.append("]");
        return sb.toString();
    }
    
    @Override
    public void y4() {
        if (this.P != null) {
            return;
        }
        final io.realm.a.h h = io.realm.a.W.get();
        this.O = (b)h.c();
        (this.P = new c0<u4.a>(this)).r(h.e());
        this.P.s(h.f());
        this.P.o(h.b());
        this.P.q(h.d());
    }
    
    public static final class a
    {
        public static final String a = "RealmParentDay";
    }
    
    static final class b extends c
    {
        long e;
        long f;
        long g;
        long h;
        long i;
        
        b(final OsSchemaInfo osSchemaInfo) {
            super(5);
            final OsObjectSchemaInfo b = osSchemaInfo.b("RealmParentDay");
            this.e = this.b("id", "id", b);
            this.f = this.b("name", "name", b);
            this.g = this.b("start", "start", b);
            this.h = this.b("end", "end", b);
            this.i = this.b("appointments", "appointments", b);
        }
        
        b(final c c, final boolean b) {
            super(c, b);
            this.d(c, this);
        }
        
        @Override
        protected final c c(final boolean b) {
            return new b(this, b);
        }
        
        @Override
        protected final void d(final c c, final c c2) {
            final b b = (b)c;
            final b b2 = (b)c2;
            b2.e = b.e;
            b2.f = b.f;
            b2.g = b.g;
            b2.h = b.h;
            b2.i = b.i;
        }
    }
}
